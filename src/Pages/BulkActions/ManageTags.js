...
const ManageTags = ({ selectedAccounts, onClose }) => {
    ...
    // Separate tags by their actions
    const assignToAllTags = Object.keys(tagActions).filter(
        (tagId) => tagActions[tagId] === "Assign to all"
    );

    const removeFromAllTags = Object.keys(tagActions).filter(
        (tagId) => tagActions[tagId] === "Remove from all"
    );
    ...
    const sendbulkTags = () => {
        ...
        fetch(`http://127.0.0.1:7000/assignbulktags/tomultipleaccount`, requestOptions)
    ...
    const removebulkTags = () => {
        ...
        fetch(`http://127.0.0.1:7000/assignbulktags/removetags`, requestOptions)
            .then((response) => response.text())
            .then((result) => {
                console.log(result)
                toast.success("Tags Assign Successfully")
