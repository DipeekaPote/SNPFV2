...
import {
    Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Select, MenuItem, Button,
    Typography, Box
} from '@mui/material';
...
const ManageTeams = ({ selectedAccounts, onClose }) => {
    ...
    const assignToAllTeamMember = Object.keys(tagActions).filter(
        (userId) => tagActions[userId] === "Assign to all"
    );

    const removeFromAllTeamMember = Object.keys(tagActions).filter(
        (userId) => tagActions[userId] === "Remove from all"
    );
    ...
    const sendbulkTeamMember = () => {
        ...
        fetch("http://127.0.0.1:7000/manageteammember/teamMembertomultipleaccount", requestOptions)
    ...
    const removebulkTeamMember = () => {
        ...
        fetch(`http://127.0.0.1:7000/manageteammember/removeteammember`, requestOptions)
            .then((response) => response.text())
            .then((result) => {
                console.log(result)
